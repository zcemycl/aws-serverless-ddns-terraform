...
def retrieve_dns_record(subdomain_name: str):
    response = dynamodb.get_item(
        TableName=TABLE_NAME, Key={"subdomain_name": {"S": subdomain_name}}
    )
    return response
...
def create_record_from_scratch(base_zone_id: str, subdomain_name: str, ip: str):
    resp = r53.create_hosted_zone(
        Name=subdomain_name, CallerReference=subdomain_name.replace(".", "")
    )
...
def delete_existing_record(base_zone_id: str, sub_zone_id: str, subdomain_name: str):
    records = r53.list_resource_record_sets(
        HostedZoneId=base_zone_id, StartRecordName=subdomain_name, StartRecordType="NS"
    )
    print(records)
    resp1 = r53.change_resource_record_sets(
        ChangeBatch={
    ...
    records2 = r53.list_resource_record_sets(
        HostedZoneId=sub_zone_id, StartRecordName=subdomain_name, StartRecordType="A"
    )
    print(records2)
    resp2 = r53.change_resource_record_sets(
        ChangeBatch={
...
def lambda_handler(event, context):
    ...
    if "method" not in event["queryStringParameters"]:
        return {
            "statusCode": 422,
            "body": "missing query string parameters -- method get del set",
        }
    if "subdomain" not in event["queryStringParameters"]:
        return {
            "statusCode": 422,
            "body": "missing query string parameters -- subdomain {xxx}.freecaretoday.com",
        }
    ...
    if method in ["set"] and ip == None:
        return {
            "statusCode": 422,
            "body": "missing query string parameters -- ip and some methods are not compatible",
        }
    if method not in allowed_modes:
        return {
            "statusCode": 422,
            "body": "wrong query string parameters -- method get del set",
        }
    ...
    if method == "set":
        ...
        if "Item" not in sub_resp:
            return {
                "statusCode": 404,
                "body": f"GET Subdomain {ddns_record} not found. ",
            }
        elif "Item" in sub_resp:
            records = r53.list_resource_record_sets(
        ...
        if "Item" not in sub_resp:
            return {
                "statusCode": 404,
                "body": f"DEL Subdomain {ddns_record} not found. ",
            }
